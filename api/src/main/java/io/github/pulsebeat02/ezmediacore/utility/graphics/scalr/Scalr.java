 ...
 *
 * <p>This class also implements an optimized version of the incremental scaling algorithm presented
 * by Chris Campbell in his <a href="http://today.java
...
public final class Scalr {
  ...
  /**
   * Flag used to indicate if debugging output has been enabled by setting the "<code>imgscalr.debug
   ...
   *     exceptions bubble up from the inside of most of the {@link BufferedImageOp} implementations
   *     and are explicitly defined on the imgscalr API to make it easier for callers to catch the
   *     exception (if they are passing along optional ops to be applied). imgscalr takes detailed
   *     steps to avoid the most common pitfalls that will cause {@link BufferedImageOp}s to fail,
   *     even when using straight forward JDK-image operations.
   */
   ...
   * @param ops <code>0</code> or more ops to apply to the image. If <code>null</code> or empty then
   ...
   *     the bounds of the <code>src</code> image (e.g. negative or too big).
   * @throws ImagingOpException if one of the given {@link BufferedImageOp}s fails to apply. These
   *     exceptions bubble up from the inside of most of the {@link BufferedImageOp} implementations
   *     and are explicitly defined on the imgscalr API to make it easier for callers to catch the
   *     exception (if they are passing along optional ops to be applied). imgscalr takes detailed
   ...
   * <code>color</code>) on the top, bottom, left and right sides of the resulting image causing the
   ...
   * @param ops <code>0</code> or more optional image operations (e.g. sharpen, blur, etc.) that can
   ...
   *     image, either fitting the image to the given width ({@link Mode#FIT_TO_WIDTH}) or fitting
   *     the image to the given height ({@link Mode#FIT_TO_HEIGHT}). If {@link Mode#AUTOMATIC} is
   *     passed in, imgscalr will calculate proportional dimensions for the scaled image based on
   *     its orientation (landscape, square or portrait). Unless you have very specific size
   *     requirements, most of the time you just want to use {@link Mode#AUTOMATIC} to "do the right
   *     thing".
   * @param targetSize The target width and height (square) that you wish the image to fit within.
   * @param ops <code>0</code> or more optional image operations (e.g. sharpen, blur, etc.) that can
   *     be applied to the final result before returning the image.
   ...
   * @return a new {@link BufferedImage} representing the scaled <code>src</code> image.
   * @throws IllegalArgumentException if <code>src</code> is <code>null</code>.
   * @throws IllegalArgumentException if <code>targetWidth</code> is &lt; 0 or if <code>targetHeight
   *     </code> is &lt; 0.
   * @throws ImagingOpException if one of the given {@link BufferedImageOp}s fails to apply. These
   *     exceptions bubble up from the inside of most of the {@link BufferedImageOp} implementations
   *     and are explicitly defined on the imgscalr API to make it easier for callers to catch the
   ...
   *     string syntax</a> that will be logged.
   ...
   *
   * <p>We force all rendering results into one of these two types, avoiding the case where a source
   ...
   *
   * <p>This method is meant to be used by any calling code (imgscalr's or otherwise) to convert any
   ...
   * targeted dimensions.
   *
   * <p>This method is intended to be used to select a specific scaling {@link Method} when a {@link
   ...
   * @param ratio A height/width ratio used to determine the orientation of the image so the primary
   *     dimension (width or height) can be selected to test if it is greater than or less than a
